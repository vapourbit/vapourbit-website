...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number, href: string) => {
    ...
    // Internal state update only if not controlled
    if (externalIndex === undefined) {
      if (internalIndex === index) return;
      setInternalIndex(index);
    }

    // Trigger visuals immediately for feedback
    if (liEl && filterRef.current) {
        ...
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`rounded-full relative cursor-pointer transition-all duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-white/70 hover:text-white ${activeIndex === index ? 'active !text-white' : ''
                  }`}
              >
                <a
                  href={item.href}
