...
class DiscGeometry extends Geometry {
    // Optimization: reduced default steps (vertices per disc)
...
function resizeCanvasToDisplaySize(canvas: HTMLCanvasElement): boolean {
    // Optimization: Cap DPR at 1.5 for performance
...
class InfiniteGridMenu {
    ...
    // Add a callback for when textures are loaded
    constructor(
        private canvas: HTMLCanvasElement,
        private items: MenuItem[],
        private onActiveItemChange: ActiveItemCallback,
        private onMovementChange: MovementChangeCallback,
        private onLoadingComplete: () => void, // Add this
        onInit?: InitCallback,
        scale: number = 1.0
    ) {
    ...
    private init(onInit?: InitCallback): void {
        ...
        // Optimization: Reduced geometry complexity
    ...
    private initTexture(): void {
        ...
        Promise.all(
            this.items.map(
                item =>
                    new Promise<HTMLImageElement>(resolve => {
                        ...
                        // Handle loading error in case image fails
                        img.onerror = () => {
                            // Create fallback placeholder
                            const placeholder = new Image();
                            // small transparent pixel or placeholder
            ...
            // Notify texture loading complete
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
    ...
    useEffect(() => {
        ...
        if (canvas) {
            sketch = new InfiniteGridMenu(
                ...
                handleActiveItem,
                setIsMoving,
                handleLoadingComplete, // Passed callback
                sk => sk.run(),
                scale
            );
        }
    ...
    const handleButtonClick = () => {
        ...
        if (activeItem.link.startsWith('http')) {
            ...
            // Assume internal router exists or default to window.location
            // We use window.location.href here as a generic fallback since useRouter isn't imported from Next.js yet
            // In the integration step we can swap this for Next.js router if needed, or just let it use standard navigation
            window.location.href = activeItem.link;
        }
    };

    return (
        <div className="relative w-full h-full" role="region" aria-label="Interactive project gallery" tabIndex={0}>
            {/* Loading Spinner */}
            {isLoading && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-20 pointer-events-none">
                    <div className="text-center">
                        <div className="w-16 h-16 border-4 border-[#00d4ff] border-t-transparent rounded-full animate-spin mb-4 mx-auto"></div>
                        <p className="text-white text-lg font-orbitron tracking-widest">Loading Works...</p>
                    </div>
                </div>
            )}
            ...
            <canvas
                ...
                ref={canvasRef}
                className={`cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
            />
            ...
            {activeItem && !isLoading && (
              ...
              pointer-events-none
              ${isMoving
                                ? 'opacity-0 duration-[100ms] translate-x-[-20%]'
            ...
            `}
              ...
              pointer-events-none
              ${isMoving
                                ? 'opacity-0 duration-[100ms] translate-x-[20%] -translate-y-1/2'
            ...
            `}
              ...
              ${isMoving
                                ...
                                : 'bottom-[10%] md:bottom-[15%] opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
                            }
            `}
                    >
                        <p className="select-none relative text-black font-bold text-[32px] md:text-[36px]">→</p>
                    </div>
                </>
            )}
        </div>
